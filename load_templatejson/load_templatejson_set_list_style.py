...
def load_template_from_json(json_path, docx_path):
    ...
    for i, heading in enumerate(config['headings']):
        ...
        # 创建对应样式
        style_name = f"my10标题 {level_index}"
...
if __name__ == "__main__":
    # 使用示例
    json_config_path = os.path.join(os.path.dirname(__file__), 'word_template_config.json')
    docx_file_path = 'source_outlined.docx'  # 需要确保这个文件存在
